...
public class NetworkAccess {
    ...
    private static NetworkResult hasInternetAccess(Context context) {
        ...
        if (isConnAvailable(context)) {
            ...
            TubeOptions opt = new TubeOptions.Builder()
                    .setHttpMethod(TubeOptions.HTTP_METHOD_HEAD)
                    .setAutoProcessHttpStatus(false)
                    .create();
            try {
                response = tube.connectStream(PING_ADDRESS, opt);
                String location = response.getHead("Location");
    ...
    private static void dPlus(NetworkResult networkResult) {
        do {
            ...
            TubeResponse response = null;
            try {
                response = tube.connectStream(DPLUS_ADDRESS, opt);
